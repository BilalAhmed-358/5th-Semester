...
void populate_f(int x, int y, int val, int map[SIZE][SIZE][SIZE])
{
	if(val!=0)
	{
		...
		{
			if(j!=x)
				map[j][y][val-1]=1; // checks row and assign 1 to positions except the original x
			if(j!=y)
				map[x][j][val-1]=1; // checks column and assign 1 to map
			if(j!=val-1)
				map[x][y][j]=1;
		}
		j=x-x%MINIGRIDSIZE;
		...
		int jj,kk;
		for(jj=j;jj<j+MINIGRIDSIZE;jj++)  // we are checking mini-grid
			for(kk=k;kk<k+MINIGRIDSIZE;kk++)
				if(jj!=x || kk!=y) // particular cell ke ilawa baki ko 1 krega
					map[jj][kk][val-1]=1;

	}
}

//does same work as populate but does it for dfs function
	...
	else
	{
		...
		for(val=0; val<SIZE; val++)
		{
			...
			if(forward_map[x][y][val]==0)
			{
				...
				dfs(board,forward_map,idx+1);  //backtrack
...
int **solveSudoku(int ** inp)
{
	...
	for(i=0; i<SIZE*SIZE; i++)
	{
		...
		if(inp[x][y]==0)
		{
			...
			#pragma omp critical
			ind=empty_cells++;
			//assigns the x and y position in empty cell list
