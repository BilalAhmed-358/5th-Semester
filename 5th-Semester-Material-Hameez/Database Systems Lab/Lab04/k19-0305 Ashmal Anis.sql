...
Task#01: For each department, retrieve the department no, the number of employees in the department and their average salary.

SELECT DEPARTMENT_ID, COUNT(*) AS NUMBER_OF_EMPLOYEES, AVG(SALARY) FROM EMPLOYEES GROUP BY DEPARTMENT_ID;
---------------------
...
Task#03: Write a Query to select Firstname and Hiredate of Employees Hired right after the joining of employee_ID no 110.
...
Task#05: Write a query to display the employee number, name (first name and last name) and job title for all employees whose salary 
...
Task#06 Write a query in SQL to display all the information of those employees who did not have any job in the past.

SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID NOT IN 
(SELECT DEPARTMENT_ID FROM JOB_HISTORY);
---------------------

Task#07: Display the manager number and the salary of the lowest paid employee of that manager. Exclude anyone whose manager 
is not known. Exclude any groups where the minimum salary is 2000. Sort the output is descending order of the salary.

SELECT MANAGER_ID, MIN(SALARY) FROM EMPLOYEES WHERE SALARY IN
(SELECT MIN(SALARY) FROM EMPLOYEES GROUP BY MANAGER_ID HAVING ((MIN(SALARY) <> 2000) AND MANAGER_ID IS NOT NULL))
GROUP BY MANAGER_ID ORDER BY MIN(SALARY);
---------------------

Task#08: Insert into employees_BKP as it should copy the record of the employee whose start date is ’13-JAN-01’ from job_History table.
...
Task#10: Delete the record of employees from employees_BKP who are manager and belongs to the department ‘Finance’.
...
WHERE SALARY > 20000 GROUP BY DEPARTMENT_ID HAVING COUNT(EMPLOYEE_ID) > 5;